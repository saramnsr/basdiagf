...
namespace BASEDiagAdulte.Ctrls
{
    public partial class Analyse2 : ImageCtrlAgg, IAnalyse
    {
        ...
        public string GetTextResultat()
        {
            ...
            s += "\nAngle Molaire Droit = " + string.Format("{0:0}", AngleIncisiveMolaireDroit) + "°";
            s += "\nAngle Molaire Gauche = " + string.Format("{0:0}", AngleIncisiveMolaireGauche) + "°";
        ...
        protected override void DrawTextResult(Epsitec.Common.Drawing.Graphics AggGr)
        {
            ...
            value = AngleIncisiveMolaireDroit;
            if (Math.Round(value) != 0) badorgoodcolor = badcolor; else badorgoodcolor = goodcolor;
            DrawOutlinedText(AggGr, "Angle Molaire Droit", ft, fontsize, 2, y, neutralcolor, new Epsitec.Common.Drawing.Color(1, 1, 1, 1));
            DrawOutlinedText(AggGr, string.Format("{0:0}", AngleIncisiveMolaireDroit) + "°", ft, fontsize, Firstwidth + (1 * Smallwidth), y, neutralcolor, new Epsitec.Common.Drawing.Color(1, 1, 1, 1));
            y -= 15;

            value = AngleIncisiveMolaireGauche;
            if (Math.Round(value) != 0) badorgoodcolor = badcolor; else badorgoodcolor = goodcolor;
            DrawOutlinedText(AggGr, "Angle Molaire Gauche", ft, fontsize, 2, y, neutralcolor, new Epsitec.Common.Drawing.Color(1, 1, 1, 1));
            DrawOutlinedText(AggGr, string.Format("{0:0}", AngleIncisiveMolaireGauche) + "°", ft, fontsize, Firstwidth + (1 * Smallwidth), y, neutralcolor, new Epsitec.Common.Drawing.Color(1, 1, 1, 1));
            y -= 15;
        ...
        private void Analyse2_Load(object sender, EventArgs e)
        {
