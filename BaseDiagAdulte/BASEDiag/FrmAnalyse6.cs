    ...
    public partial class FrmAnalyse6 : FormScreen
    {
        ...
        private void RegenerateDiagObjTraitmnt()
        {
            ...
            foreach (CommonDiagnostic cd in ResumeCliniqueMgmt.resumeCl.diagnostics)
                if (cd!=null)lstBxDiag.Items.Add(cd);
        ...
        public void InitLoad()
        {
            ...
            imageCtrl1.TextIfNoImage = ConfigurationManager.AppSettings["Attr_Portrait"] + "," + ConfigurationManager.AppSettings["Attr_Face"];
                        
            imageCtrl1.loadRadio(ResumeCliniqueMgmt.resumeCl.Img_Ext_Sourire);
            imageCtrl1.zoomAuto();
            imageCtrl1.Center();
            imageCtrl1.StartSaisie();

            barrePatient1.patient = CurrentPat;
            LoadPPT("Analyse5");

            int idx = 1;
            foreach (PointToTake ptt in ResumeCliniqueMgmt.resumeCl.LstPtAnalyseSourire)
            {
               PointToTake pt = imageCtrl1.ListOfPoints.Where( X => X.PtName == ptt.PtName ).FirstOrDefault();
               if (pt != null)
               {
                   pt.Pt = ptt.Pt;
                   pt.visible = true;
                   imageCtrl1.CurrentPointIdx = idx;
               }
               idx++;
            }

            if (imageCtrl1.CurrentPointIdx >= imageCtrl1.ListOfPoints.Count)
                imageCtrl1.EndSaisie();
            InitDisplay();
        }
        ...
        private void CheckDiags()
        {
            ...
            int[] AcceptedDiags = new int[] { 35, 36, 37 };
        ...
        {
            string file = System.Configuration.ConfigurationManager.AppSettings["CourrierAnalyseSourire"];
            if (!File.Exists(file))
            {
        ...
        public string GetAnalyseImage()
        {
            ...
            Graphics g = Graphics.FromImage(bmp);
            imageCtrl1.PaintOn(g, new Rectangle(0, 0, imageCtrl1.Width, imageCtrl1.Height), true);
        ...
        private void rbShowReel_CheckedChanged(object sender, EventArgs e)
        {

        }

        private void rbShowReel_Click(object sender, EventArgs e)
        {
            imageCtrl1.ShowReel = true;
            imageCtrl1.Invalidate();
        }

        private void rbShowTheo_Click(object sender, EventArgs e)
        {
            imageCtrl1.ShowReel = false;
            imageCtrl1.Invalidate();
        }

        private void imageCtrl1_OnEndSaisie(object sender, EventArgs e)
        {
            ResumeCliniqueMgmt.resumeCl.LstPtAnalyseSourire = imageCtrl1.ListOfPoints;
        }
    }
}
