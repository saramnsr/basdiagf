...
namespace BaseCommonControls
{
    public class DataGridViewPrinter
    {
        ...
        public DataGridViewPrinter(DataGridView aDataGridView, PrintDocument aPrintDocument, bool CenterOnPage, bool WithTitle, string aTitleText, Font aTitleFont, Color aTitleColor, bool WithPaging)
        {
            ...
            LeftMargin = ThePrintDocument.DefaultPageSettings.Margins.Left -170;
            TopMargin = ThePrintDocument.DefaultPageSettings.Margins.Top;
            RightMargin = ThePrintDocument.DefaultPageSettings.Margins.Right - 50;
        ...
        private void Calculate(Graphics g)
        {
            if (PageNumber == 0) // Just calculate once
            {
                ...
                if (TheDataGridView.AutoSizeColumnsMode == DataGridViewAutoSizeColumnsMode.Fill)
                {
                    TheDataGridViewWidth = TheDataGridView.Width;

                    float totalfillweight = 0;
                    for (int i = 0; i < TheDataGridView.Columns.Count; i++)
                        totalfillweight += TheDataGridView.Columns[i].FillWeight;
                    ...
                    for (int i = 0; i < TheDataGridView.Columns.Count; i++)
                    {
                        ...
                        tmpWidth = (TheDataGridViewWidth-LeftMargin-RightMargin) * (TheDataGridView.Columns[i].FillWeight/totalfillweight);
                        ColumnsWidth.Add(tmpWidth + 20);
                ...
                if ((TheDataGridView.AutoSizeColumnsMode == DataGridViewAutoSizeColumnsMode.AllCells)||
                    (TheDataGridView.AutoSizeColumnsMode == DataGridViewAutoSizeColumnsMode.AllCellsExceptHeader) ||
                    (TheDataGridView.AutoSizeColumnsMode == DataGridViewAutoSizeColumnsMode.DisplayedCells) ||
                    (TheDataGridView.AutoSizeColumnsMode == DataGridViewAutoSizeColumnsMode.DisplayedCellsExceptHeader))
                {
                    TheDataGridViewWidth = 0;
                    int start = ((TheDataGridView.AutoSizeColumnsMode == DataGridViewAutoSizeColumnsMode.AllCellsExceptHeader) || (TheDataGridView.AutoSizeColumnsMode == DataGridViewAutoSizeColumnsMode.DisplayedCellsExceptHeader)) ? 1 : 0;

                    for (int i = start; i < TheDataGridView.Columns.Count; i++)
                    {
        ...
        // When it returns false, meaning that all rows are printed (the CurrentRow parameter reaches the last row of the DataGridView control) and no further PagePrint action is required
